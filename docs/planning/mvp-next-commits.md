...
## NOW
- [ ] `audit(reverify): confirm upload endpoint auth + MIME/size limits with regression test coverage`
- [ ] `audit(reverify): confirm LIVE consentTextVersion persistence across schema/DTO/API/audit flow`
- [ ] `feat(stream-fanout): wire server-owned SSE fan-out endpoint from market-stream worker to dashboard`
